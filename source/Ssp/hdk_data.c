...
static int HDK_Type__STRING__DESERIALIZE__(HDK_Member* pMember, char* pszValue)
{
    ...
    {
        /* CID 65341 Calling risky function */
        strncpy(((HDK_Member__STRING__*)pMember)->pszValue, pszValue,cchValue);
        ((HDK_Member__STRING__*)pMember)->pszValue[cchValue] = '\0';
        return 1;
    }
    else
    {
...
HDK_Member* HDK_Append_String(HDK_Struct* pStruct, HDK_Element element, char* pszValue)
{
    ...
    if (pMember)
    {
        ...
        if (pMember->pszValue)
        {
            /* CID 74655 Calling risky function */
            strncpy(pMember->pszValue, pszValue, cchValue);
            pMember->pszValue[cchValue] = '\0';
            HDK_Append_Member(pStruct, (HDK_Member*)pMember);
        }
        else
        {
    ...
    if (pActions)
    {
        ...
        for (pMethod = s_methods; pMethod != pMethodEnd; pMethod++)
        {
            ...
            if (!(pMethod->prop & HDK_MethodProp_Hidden))
            {
                ...
                strncpy(szSOAPAction, s_namespaces_GetString(pElemNode->ixNamespace), sizeof(szSOAPAction) - 1);
                if (szSOAPAction[strlen(szSOAPAction) - 1] != '/')
                {
                    strncat(szSOAPAction, "/", sizeof(szSOAPAction) - strlen(szSOAPAction) - 1);
                }
                strncat(szSOAPAction, pElemNode->pszElement, sizeof(szSOAPAction) - strlen(szSOAPAction) - 1);
