...
/* CID 135309 Large stack use */
static char insPath[MAX_INSTANCE][MAX_PATH_NAME];
...
int HDK_Device_GetValue(void* pDeviceCtx, HDK_Struct* pStruct, HDK_DeviceValue eValue, HDK_Struct* pInput)
{
    ...
    /* CID 135587 Large stack use */
    memset(insPath, '\0', sizeof(insPath));
    ...
    switch (eValue)
    {
        ...
        case HDK_DeviceValue_PortMappings:
	     ...
	     {
                /* CID 66233 Calling risky function */
                snprintf(tmpBuf,sizeof(tmpBuf),"%s", insPath[i]);
                snprintf(path,sizeof(path), "%sInternalPort", tmpBuf);
                ...
                snprintf(path,sizeof(path),"%sInternalClient", tmpBuf);
                if (MBus_GetParamVal(mbus, path, val, sizeof(val)) != 0)
                    return 0;
                ...
                snprintf(path,sizeof(path),"%sProtocol", tmpBuf);
                if (MBus_GetParamVal(mbus, path, val, sizeof(val)) != 0)
                    return 0;
				...
				*/
                ...
                snprintf(path,sizeof(path),"%sDescription", tmpBuf);
                if (MBus_GetParamVal(mbus, path, val, sizeof(val)) != 0)
                    return 0;
                ...
                snprintf(path,sizeof(path),"%sExternalPort", tmpBuf);
                if (MBus_GetParamVal(mbus, path, val, sizeof(val)) != 0)
                    return 0;
        ...
        case HDK_DeviceValue_PresentationURL:
			snprintf(val,sizeof(val),"%s","http://");
			//Get instance 
...
/*
        ...
        case HDK_DeviceValue_WanGateway:
            ...
            if (MBus_GetParamVal(mbus, tmpPath, val, sizeof(val)) != 0)
                return 0;
			if (!strcmp(val, ""))
				snprintf(val, sizeof(val), "%s", "0.0.0.0");
			if (sscanf(val, "%hhu.%hhu.%hhu.%hhu", 
                            &ipAddr.a, &ipAddr.b, &ipAddr.c, &ipAddr.d) != 4)
				return 0;
    ...
    switch (eValue)
    {
        case HDK_DeviceValue_PortMappings:
            ...
            if (HDK_Get_Int(pStruct, HDK_Element_PN_InternalPort) != NULL && i >= insNum)
            {
                ...
                snprintf(insPath[i],MAX_PATH_NAME,"Device.NAT.PortMapping.%d.", newIns);
            }
            ...
            {   
				...
				/* CID 61560 Dereference null return value */
				if(ipAddr == NULL)
				{
				    log_printf(LOG_ERR, "Failed to get Internal client IP address\n");
				    return 0;
				}
				...
				snprintf(val,sizeof(val), "%u", *intVal);
				...
				snprintf(tmp,sizeof(tmp),"%u", *intVal);
        ...
        case HDK_DeviceValue_WLanChannelWidth:
			...
			if (*intVal == 0)
			{
				snprintf(tmpBuf, sizeof(tmpBuf),"%s","Auto");
			}
			else if (*intVal == 20 || *intVal == 40)
			{
				snprintf(tmpBuf, sizeof(tmpBuf), "%dMHz", *intVal);
			}
			else
				return 0;
        ...
        case HDK_DeviceValue_WLanChannel:
            ...
            {
                snprintf(tmpBuf,sizeof(tmpBuf), "%d", *intVal);
                snprintf(tmpPath, sizeof(tmpPath), "%s.Channel", pathVal);
        ...
        case HDK_DeviceValue_WLanKeyRenewal:
            ...
            snprintf(tmpBuf,sizeof(tmpBuf), "%u", *intVal);
        ...
        case HDK_DeviceValue_WLanKey:
            ...
            /* CID 73324: (#3 of 3): Dereference null return value */
            if(boolVal == NULL)
            {
                log_printf(LOG_ERR, "BoolMember is NULL  \n");
                return 0;
            }
        ...
        case HDK_DeviceValue_DHCPReservations:
            ...
            for (pMember = pStr1->pHead; pMember != NULL; pMember = pMember->pNext)
            {
                ...
                snprintf(path,sizeof(path), "%s.StaticAddress.%d.Yiaddr", pathVal, insNum);
                ...
                if (MBus_SetParamVal(mbus, path, MBUS_PT_STRING, ipBuf, 0) != 0)
                    return 0;
                snprintf(path,sizeof(path), "%s.StaticAddress.%d.Chaddr", pathVal, insNum);
                ...
                if (MBus_SetParamVal(mbus, path, MBUS_PT_STRING, macBuf, 0) != 0)
                    return 0;
                /* CID 72812 Calling risky function */
		snprintf(path,sizeof(path), "%s.StaticAddress.%d.X_CISCO_COM_DeviceName", pathVal, insNum);
...
int HDK_Stop_HomeSecurity_Bridge(MBusObj_t *ccsp_bus)
{
	...
	/* CID 54762 Calling risky function */
	strncat(path, ".Port.",HS_STRLEN);
...
int HDK_Start_HomeSecurity_Bridge(MBusObj_t *ccsp_bus)
{
	...
	/* CID 53097 Calling risky function */
	strncat(path, ".Port.",HS_STRLEN);
	//Get port 4 bridge port number
	if (MBus_GetParamVal(ccsp_bus, "Device.X_CISCO_COM_MultiLAN.PrimaryLANBridgeHSPorts", path + strlen(path), sizeof(path) - strlen(path)) != 0)
	{
