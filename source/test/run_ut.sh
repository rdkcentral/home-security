...
log "INFO" "Running make for Gtest..."
...
if [ -f "./source/test/CcspHomeSecurity_gtest.bin" ]; then
    log "INFO" "Running CcspHomeSecurity_gtest.bin"
    ./source/test/CcspHomeSecurity_gtest.bin
    log "INFO" "Completed Test Execution of CcspHomeSecurity_gtest.bin"
else
    log "ERROR" "CcspHomeSecurity_gtest.bin does not exist, cannot run tests"
    exit 1
fi
